...
import { UserAlreadyExistsError, UserNotFoundError , UnauthorizedUserError,UserIsAdminError,UserNotAdminError ,InvalidUpdateError} from "../errors/userError";
...
class UserController {
    ...
    async deleteUser(user: User, username: string): Promise<boolean> {
        try {
            if (user.role !== Role.ADMIN && username !== user.username) {
                throw new UserNotAdminError(); // Customer cannot delete another user
            }
            const userr = await this.dao.getUserByUsername(username);
            if (user.role === Role.ADMIN) {
                if (userr.role !== Role.ADMIN || userr.username === user.username) {
                    return await this.dao.deleteUser(username);
                } else {
                    throw new UserIsAdminError();
                }
            } else { // If not an admin, only allowed to delete own account
                return await this.dao.deleteUser(username);
            }
        } catch (err) {
            throw err; 
    ...
    async updateUserInfo(user: User, name: string, surname: string, address: string, birthdate: string, username: string): Promise<User> {
        try {
            ...
            const userr = await this.dao.getUserByUsername(username);
            if (user.role === Role.ADMIN) {
                if (userr.role !== Role.ADMIN) {
                    return await this.dao.updateUserInfo(userr, name, surname, address, birthdate);
                } else if (user.username === username) {
                    return await this.dao.updateUserInfo(userr, name, surname, address, birthdate);
                } else if (user.username !== username && userr.role === Role.ADMIN) {
                    throw new UserIsAdminError();
                }
            } else {
                if (username === user.username) {
                    return await this.dao.updateUserInfo(user, name, surname, address, birthdate);
                } else {
