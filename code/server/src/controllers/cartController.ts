...
class CartController {
    ...
    async addToCart(user: User, productId: string) : Promise<Boolean> {
        return this.dao.addProduct(user, productId);
    }
    ...
    async getCart(user: User) : Promise<Cart> {
        return this.dao.getCurrentUserCart(user);
    }
    ...
    async getCustomerCarts(user: User) :Promise<Cart[]>  {
        return this.dao.getPaidUserCarts(user);
    } 
    ...
    async removeProductFromCart(user: User, product: string) :Promise<Boolean>  {
        return this.dao.removeProduct(user, product);
    }
