...
class ProductController {
    ...
    async registerProducts(model: string, category: string, quantity: number, details: string | null, sellingPrice: number, arrivalDate: string | null): Promise<void> {
        return new Promise<void>((resolve, reject) => {
            if (dayjs(arrivalDate).isAfter(dayjs())) {
                ...
                this.dao.registerProducts(model, category, quantity, details, sellingPrice, arrivalDate)
                    .then(() => resolve())
                    .catch(reject);
            }
        });
    ...
    async changeProductQuantity(model: string, newQuantity: number, changeDate: string | null): Promise<number> {
        return new Promise<number>((resolve, reject) => {
            if (dayjs(changeDate).isAfter(dayjs())) {
                ...
                this.dao.changeProductQuantity(model, newQuantity, changeDate)
                    .then((newQty) => resolve(newQty))
                    .catch(reject);
            }
        });
    }
        ...
        return new Promise<Product[]>((resolve, reject) => {
            if (!grouping) {
                if (!category && !model) {
                    ...
                    return reject(new GetProductsError);
                }
            } else if (grouping === 'category') {
                if (category && !model) {
                    this.dao.getProductsByCategory(category)
                        .then(products => resolve(products))
                        .catch(reject);
                } else {
                    return reject(new GetProductsError);
                ...
                if (model && !category) {
                    this.dao.getProductByModel(model)
                        .then(products => resolve(products))
                        .catch(reject);
                } else {
                    return reject(new GetProductsError);
    ...
    async getAvailableProducts(grouping: string | null, category: string | null, model: string | null): Promise<Product[]> {
        return new Promise<Product[]>((resolve, reject) => {
            if (!grouping) {
                if (!category && !model) {
                    this.dao.getAllProducts()
                        .then(products => resolve(products.filter(product => product.quantity > 0)))
                        .catch(reject);
                } else {
                    return reject(new GetProductsError);
                }
    ...
    async deleteAllProducts() /**:Promise <Boolean> */ { 
        return await this.dao.deleteProduct(null)
    }
