...
class UserRoutes {
    ...
    initRoutes() {
        ...
        this.router.post(
            "/",
            body("username").exists({checkNull: true, checkFalsy: true}).isString().isLength({ min: 1 }),
            body("name").exists({checkNull: true, checkFalsy: true}).isString().isLength({ min: 1 }),
            body("surname").exists({checkNull: true, checkFalsy: true}).isString().isLength({ min: 1 }),
            body("password").exists({checkNull: true, checkFalsy: true}).isString().isLength({ min: 1 }),
            body("role").exists({checkNull: true, checkFalsy: true}).isString().isIn(["Manager", "Customer", "Admin"]),
        ...
        this.router.get(
            ...
            this.authService.isLoggedIn,
            this.authService.isAdmin,
            param("role").exists({checkNull: true, checkFalsy: true}).isString().isIn(["Manager", "Customer", "Admin"]),
            this.errorHandler.validateRequest,
            (req: any, res: any, next: any) =>
                this.controller
        ...
        this.router.get(
            "/:username",
            this.authService.isLoggedIn,
            param("username").exists({checkNull: true, checkFalsy: true}).isString().notEmpty(),
            this.errorHandler.validateRequest,
            (req: any, res: any, next: any) =>
                this.controller
        ...
        this.router.patch(
            ...
            body("name").exists({checkNull: true, checkFalsy: true}).isString().isLength({ min: 1 }),
            body("surname").exists({checkNull: true, checkFalsy: true}).isString().isLength({ min: 1 }),
            body("address").exists({checkNull: true, checkFalsy: true}).isString().isLength({ min: 1 }),
            body("birthdate").exists({checkNull: true, checkFalsy: true}).isString().isDate({format:"YYYY-MM-DD"}),
            param("username").exists({checkNull: true, checkFalsy: true}).isString().notEmpty(),
            this.errorHandler.validateRequest,
            (req: any, res: any, next: any) =>
                this.controller
                    .updateUserInfo(req.user, req.body.name, req.body.surname, req.body.address, req.body.birthdate, req.params.username)
                    .then((user: any /**User */) => res.status(200).json(user))
                    .catch((err: any) => next(err))
        );
